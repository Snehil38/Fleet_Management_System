...
//    }
    ...
    func fetchAllTrips() async throws {
        ...
        do {
            // Use the existing fetchTrips method which already has the correct implementation
            try await fetchTrips()
            
            // Update published properties on the main thread
            await MainActor.run {
                // Filter trips by status
                self.upcomingTrips = self.allTrips.filter { $0.status == .pending || $0.status == .assigned }
                
                if self.driverId != nil {
                    self.currentTrip = self.allTrips.first(where: { $0.status == .inProgress })
                }
